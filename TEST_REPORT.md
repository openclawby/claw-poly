...
- 新增安全测试：44。
- 最终：53 total / 53 passed / 0 failed / 0 skipped / 2 warnings。
- 全新隔离环境用时：2.95 秒。

```text
.....................................................                    [100%]
53 passed, 2 warnings in 2.95s
```

警告：FastAPI TestClient 的 Starlette/httpx 弃用提示；原测试 `asyncio.get_event_loop()` 弃用提示。两者均不是测试失败。

覆盖：固定 paper mode、凭据启动拒绝、Clawby allow、无 Clawby 受限启动、CLOB client fail closed、模拟下单/止盈/撤单、非模拟 ID 拒绝、redeem fail closed、敏感路由 404、settings 敏感字段/类型/范围、非字符串 strategy 拒绝、整数参数小数拒绝、极大整数溢出与非有限数值拒绝、Host/Origin、旧 DB 清理、前端敏感控件和源代码执行调用缺失。
